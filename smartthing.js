...
function https_request(authToken, method, path, data, callback) {
  ...
  var request = https.request(post_options, function(response) {
      ...
      response.on('data', function(chunk) {
        response_data.push(chunk.toString());
      });

      response.on('end', function() {
        var data = response_data.join('');
        try {
          data = JSON.parse(response_data.join(''));
        } catch (e) {
          console.log('Failed to convert response to JSON (%d)', status_code);
          return;
        }
...
exports.device_list = function(options, callback) {
  var path = '/devices?locationId=' + options.locationId;
  https_request(options.authToken, 'GET', path, '', callback);
};
...
exports.device_events = function(options, deviceEvents, callback) {
  https_request(options.authToken, 'POST',
                '/devices/' + options.deviceId + '/events',
                JSON.stringify(deviceEvents), callback);
};
